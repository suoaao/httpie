...
class TestImplicitHTTPMethod:
    ...
    def test_implicit_POST_stdin(self, httpbin):
        with open(FILE_PATH) as f:
            ...
            r = http('--form', f'{httpbin.url}/post', env=env)
        assert HTTP_OK in r
...
class TestAutoContentTypeAndAcceptHeaders:
    ...
    def test_GET_no_data_no_auto_headers(self, httpbin):
        ...
        r = http('GET', f'{httpbin.url}/headers')
        assert HTTP_OK in r
    ...
    def test_POST_no_data_no_auto_headers(self, httpbin):
        ...
        r = http('POST', f'{httpbin.url}/post')
        assert HTTP_OK in r
        assert '"Accept": "*/*"' in r
        assert '"Content-Type": "application/json' not in r

    def test_POST_with_data_auto_JSON_headers(self, httpbin):
        r = http('POST', f'{httpbin.url}/post', 'a=b')
        assert HTTP_OK in r
    ...
    def test_POST_explicit_JSON_auto_JSON_accept(self, httpbin):
        r = http('--json', 'POST', f'{httpbin.url}/post')
        assert HTTP_OK in r
        assert r.json['headers']['Accept'] == JSON_ACCEPT
    ...
    def test_print_overridable_when_stdout_redirected(self, httpbin):
        env = MockEnvironment(stdin_isatty=True, stdout_isatty=False)
        r = http('--print=h', 'GET', f'{httpbin.url}/get', env=env)
        assert HTTP_OK in r
